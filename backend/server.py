...
def run_analysis(job_id: str, run_dir: Path, input_dir: Path, config: dict) -> None:
    ...
    try:
        ...
        args = [
            ...
            "--min-genes", str(config["minGenes"]), "--max-genes", "6000",
            "--max-counts", str(config["maxCounts"]), "--max-mito", str(config["maxMito"]),
            "--max-ribo", str(config["maxRibo"]), "--min-cells-per-gene", str(config["minCellsPerGene"]),
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            config = {
                ...
                "minMito": float(config.get("maxMito", 20)),
